...
set adaptfn [file join $adaptdir cylinder_results.vtu]
...
catch {repos_delete -obj adaptOutflow}
repos_readXMLPolyData [file join $adaptdir mesh-complete mesh-surfaces outlet.vtp] adaptOutflow
set adaptNumPts [geom_numPts -obj adaptOutflow]
set adaptOutflowObj [repos_exportToVtk -src adaptOutflow]
set adaptOutflowScalars [[$adaptOutflowObj GetPointData] GetScalars]
...
$resReader Update
puts "Reading simulation results ($adaptfn)."
set adaptReader tmp-results-adapt
catch {$adaptReader Delete}
vtkXMLUnstructuredGridReader $adaptReader
$adaptReader SetFileName $adaptfn
$adaptReader Update

# now calculate flow rate for each time point
set myVectors tmp-myVectors
set myAdaptVectors tmp-myAdaptVectors
...
foreach stepnum $simstepnumlist {
  ...
  catch {$myAdaptVectors Delete}
  vtkFloatArray $myAdaptVectors; $myAdaptVectors SetNumberOfComponents 3
  $myAdaptVectors Allocate 1000 1000

  set pointData [[$resReader GetOutput] GetPointData]
  set adaptPointData [[$adaptReader GetOutput] GetPointData]
  set objVectors [$pointData GetArray velocity_[format %05i $stepnum]]
  set adaptObjVectors [$adaptPointData GetArray velocity_[format %05i $stepnum]]
  ...
  for {set i 0} {$i < $adaptNumPts} {incr i} {
      set node [expr int([$adaptOutflowScalars GetValue $i])]
      # nodes in vtk start at 0, so subtract 1 off
      set node [expr $node - 1]
      set vec [$adaptObjVectors GetTuple3 $node]
      #puts "processing vtk node: $node vec: $vec"
      $myAdaptVectors InsertNextTuple3 [lindex $vec 0] [lindex $vec 1] [lindex $vec 2]
  }

  # associate vectors with polydata
  [$outflowObj GetPointData] SetVectors $myVectors
  [$adaptOutflowObj GetPointData] SetVectors $myAdaptVectors
  ...
  for {set j -20} {$j < 21} {incr j} {
      set r [format {%.4f} [expr double($j)/double(10)]]
      set results($stepnum,$r) {}
      catch {set results($stepnum,$r) [list $r [lindex [geom_interpolateVector -obj adaptOutflow -pt [list $r 0 0]] 2]]}
  }

  set fp [open [file join $adaptdir profiles_for_$time] w]
  fconfigure $fp -translation lf
  puts -nonewline $fp "radius\tr/R\tanalytic\t"
  puts -nonewline $fp "$stepnum\t"
  puts $fp ""
...
proc demoCreateGraph {graphname title analytic adaptsoln feasoln xoffset yoffset} {
...
Please use the "Current widget path:" to show/hide windows.}}
  ...
  $graphname data d2 -colour blue -points 1 -lines 0 -coords $adaptsoln
  $graphname data d3 -colour red -points 1 -lines 0 -coords $feasoln
  ...
  wm geometry .$guiDEMOgraph1 $newGeometry
   ...
   for {set i 0} {$i < 4} {incr i} {
    ...
    demoCreateGraph $graphname $title $analytic $adaptsoln $feasoln [expr 100+$i*50] [expr 100+$i*50]
  }
