...
#
if {$num_procs > 1} {
  puts "Running Post Solver for sol and ybar"
  if [catch {exec $POSTSOLVER -sn $adapt_step -indir $fullsimdir -outdir $fullrundir -ph -sol -ybar} msg] {
    puts $msg
...
#
puts "exec $POSTSOLVER -indir $adaptsimdir -outdir $adaptdir -start $adapt_start -stop $total_timesteps -incr 1 -sim_units_mm -vtkcombo -vtu cylinder_results.vtu -vtp cylinder_results.vtp"

if [catch {exec $POSTSOLVER -indir $adaptsimdir -outdir $adaptdir -start $adapt_start -stop $total_timesteps -incr 1 -sim_units_mm -vtkcombo -vtu cylinder_results.vtu -vtp cylinder_results.vtp} msg] {
   puts $msg
   return -code error "ERROR running postsolver!"
}
